...
class Settings(BaseSettings):
    ...
    # Candidate-1: Enhanced BG removal (PLANNED - not yet implemented in code)
    # Note: These settings are documented for future implementation
    # use_multi_threshold: bool = Field(default=True, env="USE_MULTI_THRESHOLD", description="Use multi-threshold masking for better edges")
    # min_mask_threshold: float = Field(default=0.7, env="MIN_MASK_THRESHOLD")
    # mid_mask_threshold: float = Field(default=0.8, env="MID_MASK_THRESHOLD")
    # max_mask_threshold: float = Field(default=0.9, env="MAX_MASK_THRESHOLD")
    # use_adaptive_padding: bool = Field(default=True, env="USE_ADAPTIVE_PADDING", description="Adjust padding based on object size and aspect ratio")
    # enable_morphological_ops: bool = Field(default=True, env="ENABLE_MORPHOLOGICAL_OPS", description="Clean up mask edges")
    # enable_antialiasing: bool = Field(default=True, env="ENABLE_ANTIALIASING", description="Smooth mask edges")
